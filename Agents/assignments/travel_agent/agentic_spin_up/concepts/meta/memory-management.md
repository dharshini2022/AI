...
record.toolNames = [ ... ];         // The spec's tools; their latest raw JSON results live in the shared Scratchpad
...
- Lossless Raw Tool Results: MCP tool results are intercepted live and recorded in the shared Scratchpad (see `concepts/architecture/scratchpad.md`), keyed by tool name. The downstream itinerary planner can directly access exact JSON data (flights, hotels, weather) to render cards and calculate budgets without searching or parsing the raw conversational LLM history.
