...
rather than `ask_user`, relays it with `send_message_to_subagent`, and remembers the answer as
the `indoor_mode` it reports in STEP 5. The old STEP 4 was deleted — left in place it would
...
`concepts/subagent-lifecycle.md`, Use case 1): a sub-agent ends a turn with
...
includes the fare) and both local MCP calls costing milliseconds — `transportLabels` already
